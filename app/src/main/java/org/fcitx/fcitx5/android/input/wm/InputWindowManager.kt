...
class InputWindowManager : UniqueViewComponent<InputWindowManager, FrameLayout>(),
    ...
    private var keyboardWindowKey: EssentialWindow.Key? = null
    private var hiddenKeyboardWindowKey: EssentialWindow.Key? = null
    private var keyboardWindowVisible = true
    ...
    fun registerKeyboardVisibilityWindows(
        keyboardWindowKey: EssentialWindow.Key,
        hiddenKeyboardWindowKey: EssentialWindow.Key,
        visible: Boolean = true
    ) {
        this.keyboardWindowKey = keyboardWindowKey
        this.hiddenKeyboardWindowKey = hiddenKeyboardWindowKey
        this.keyboardWindowVisible = visible
    }

    fun isKeyboardWindowVisible() = keyboardWindowVisible

    fun setKeyboardVisibilityListener(listener: (Boolean) -> Unit) {
        keyboardVisibilityListener = listener
    }

    fun setKeyboardWindowVisible(visible: Boolean) {
        if (keyboardWindowVisible == visible) return
        keyboardWindowVisible = visible
        keyboardVisibilityListener?.invoke(visible)
        val current = currentWindow
        val keyboard = keyboardWindowKey?.let { essentialWindows[it]?.first }
        val hidden = hiddenKeyboardWindowKey?.let { essentialWindows[it]?.first }
        if (current != null && (current === keyboard || current === hidden)) {
            attachWindow(resolveWindowKey(keyboardWindowKey!!))
        }
    }

    private fun resolveWindowKey(windowKey: EssentialWindow.Key): EssentialWindow.Key {
        return if (windowKey == keyboardWindowKey && !keyboardWindowVisible) {
            hiddenKeyboardWindowKey ?: windowKey
        } else {
            windowKey
        }
    }
    ...
    fun attachWindow(windowKey: EssentialWindow.Key) {
        val resolvedWindowKey = resolveWindowKey(windowKey)
        essentialWindows[resolvedWindowKey]?.let { (window, _) ->
            attachWindow(window)
        } ?: throw IllegalStateException("$resolvedWindowKey is not a known essential window key")
    }
